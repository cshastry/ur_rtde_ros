...
class Receiver {
 public:
  explicit Receiver(const std::string& base_frame, const std::string& tool_frame,
    ...
    std::transform(base_joint_names.begin(), base_joint_names.end(),
                   std::back_inserter(joint_names_), [&](const auto& n) { return prefix + n; });
  }
...
class Controller {
 ...
 public:
  explicit Controller(const std::string& base_frame, const std::string& prefix,
    ...
    if (ros::param::has("~servo_rate")) {
      ...
      if (ur_step_time == 0)
        ROS_WARN("'%s' returned step time 0, defaulting to %.2f ms (%.2f Hz)", hostname.c_str(),
                 getStepTime() * 1000, 1.0 / getStepTime());
      else
        setLoopRate(1.0 / ur_step_time);
    }
  }

  ~Controller() { rtde_ctrl_.stopScript(); }
  ...
  void setLoopRate(double f) {
    ...
    ROS_INFO("Setting control loop period to %.2f ms (%.2f Hz)", getStepTime() * 1000,
             1.0 / getStepTime());
  }
  ...
  void servoJ(const sensor_msgs::JointState& m) {
    ...
    enqueueCommand([this, q = m.position]() {
      ...
      if (!rate_.sleep())
        ROS_WARN_THROTTLE(0.5, "ServoJ cycle time overstepped (expected: %.2f ms, actual: %.2f ms)",
  ...
  void moveL(const geometry_msgs::PoseStamped& m) {
    ...
    if (m.header.frame_id != base_frame_) {
      ROS_WARN_STREAM("Discarding MoveL command with target frame " << "'" << m.header.frame_id
                                                                    << "'"
                                                                    << "(expected "
                                                                    << "'" << base_frame_ << "')");
      return;
  ...
  void speedJ(const sensor_msgs::JointState& m) {
    ...
    enqueueCommand([this, qd = m.velocity]() {
      if (state_ != State::SPEEDING) {
        state_ =
            State::SPEEDING;  // cleared if we don't keep receiving speed commands for some time
      }
...
int main(int argc, char* argv[]) {
  ...
  if (hostname.empty()) throw std::runtime_error("The ~hostname parameter is not set");

  std::vector<std::string> rtde_output_variables = {"actual_q", "actual_qd", "actual_current"};

  if (publish_tcp_pose) rtde_output_variables.push_back("actual_TCP_pose");

  if (publish_tcp_twist) rtde_output_variables.push_back("actual_TCP_speed");
  ...
  auto pub_tcp_pose = (publish_tcp_pose)
                          ? nh.advertise<geometry_msgs::PoseStamped>("tcp_pose_current", 1)
                          : ros::Publisher{};
  auto pub_tcp_twist = (publish_tcp_twist)
                           ? nh.advertise<geometry_msgs::TwistStamped>("tcp_twist_current", 1)
                           : ros::Publisher{};

  std::list<ros::ServiceServer> service_servers{
      nh.advertiseService("set_servo_joint_lookahead_time", &Controller::setServoJLookaheadTime,
                          &controller),
      nh.advertiseService("set_servo_joint_gain", &Controller::setServoJGain, &controller),
  };

  std::list<ros::Subscriber> subscribers{
      nh.subscribe("move_joint", 2, &Controller::moveJ, &controller,
                   ros::TransportHints().tcp().tcpNoDelay()),
      nh.subscribe("move_tool_linear", 2, &Controller::moveL, &controller,
                   ros::TransportHints().tcp().tcpNoDelay()),
      nh.subscribe("servo_joint", 1, &Controller::servoJ, &controller,
                   ros::TransportHints().udp().tcp().tcpNoDelay()),
      nh.subscribe("speed_joint", 1, &Controller::speedJ, &controller,
                   ros::TransportHints().udp().tcp().tcpNoDelay()),
      nh.subscribe("speed_linear", 1, &Controller::speedL, &controller,
                   ros::TransportHints().udp().tcp().tcpNoDelay()),
  ...
  auto timer = nh.createSteadyTimer(ros::WallDuration(controller.getStepTime()), [&](const auto&) {
    pub_joint_state.publish(receiver.getJointStateMsg());

    if (publish_tcp_pose) pub_tcp_pose.publish(receiver.getActualTCPPoseMsg());

    if (publish_tcp_twist) pub_tcp_pose.publish(receiver.getActualTCPTwistMsg());
  });
